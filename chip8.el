  ...
  (let ((type (ash (logand #xF000 instruction) -12))
    ...
    (cond
     ...
     ((= type #x7)
      ...
      (aset chip8-regs x (mod (+ (aref chip8-regs x) kk) 256)))
     ((= type #x8)
     ...
     ((= type #xD)
      (let ((col (aref chip8-regs x))
        ...
        (dotimes (rowo n)
          (let ((byte (aref chip8-ram (+ I rowo))))
            (dotimes (colo 8)
              (let ((power (ash 1 (- 7 colo)))
                    (idx (+ (* chip8-fb-width
                               (mod (+ row rowo) chip8-fb-height))
                            (mod (+ col colo) chip8-fb-width))))
